/**
 * Fixed Product Import Script - Handles Duplicates Properly
...
// Deduplicate products - keep the one with highest public price
function deduplicateProducts(products) {
  ...
  products.forEach(product => {
    if (productMap.has(product.code)) {
      duplicatesFound.push(product.code);
      const existing = productMap.get(product.code);
      
      // Keep the one with higher public price (more recent/accurate)
      if (product.publicPrice > existing.publicPrice) {
  ...
  console.log(`🔄 Found ${uniqueDuplicates.length} codes with duplicates:`, uniqueDuplicates.slice(0, 10));
  console.log(`✅ Deduplicated: ${products.length} → ${productMap.size} products`);
...
// Insert products with upsert to handle any remaining conflicts
async function insertProducts(products, categoryMap, dryRun = false) {
  ...
  const batchSize = 50; // Larger batches now that duplicates are handled
  let successful = 0;
  const failed = [];
  ...
  for (let i = 0; i < valid.length; i += batchSize) {
    ...
    try {
      ...
      console.log(`✅ Inserted/Updated ${batch.length} products`);
      ...
      for (const product of batch) {
        try {
          ...
          if (individualError) {
            failed.push({ product, error: individualError.message });
          } else {
            successful++;
          }
        } catch (individualErr) {
          failed.push({ product, error: individualErr.message });
        }
      }
    }
...
async function importProducts(csvPath, dryRun = false) {
  ...
  try {
    ...
    const deduplicatedProducts = deduplicateProducts(validProducts);
    
    console.log(`
📊 DATA PROCESSING RESULTS:
===========================
✅ Valid products: ${validProducts.length}
✅ After deduplication: ${deduplicatedProducts.length}
❌ Invalid products: ${invalidProducts.length}
❌ Duplicates removed: ${validProducts.length - deduplicatedProducts.length}
`);
    ...
    const categoryMap = await ensureCategories(deduplicatedProducts);
    
    // Insert products
    const results = await insertProducts(deduplicatedProducts, categoryMap, dryRun);
...
📊 Total processed: ${deduplicatedProducts.length}
`);
...
if (!csvFile) {
  console.error('Usage: node import-fixed.js <csv-file> [--dry-run]');
  console.error('Example: node import-fixed.js "../products-export.csv" --dry-run');
  process.exit(1);
}
